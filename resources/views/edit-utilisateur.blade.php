...
<body>
    ...
    <div class="main-panel" id="main-panel" style="background-color: #ffffff">
        ...
        <div class="content mt-5">
            ...
            <form class="pt-5" method="POST" action="{{ route('dashboard.update-utilisateur',$clients->id) }}">
                @csrf
                @method('PUT')
                <div class="form-group">
                  <label for="exampleFormControlInput1">Nom ET Prenom</label>
                  <input type="text" name="information" class="form-control" autocomplete="off" value="{{ $clients->information }}">
                </div>
                <div class="form-group">
                    <label for="exampleFormControlInput1">Email</label>
                    <input type="email" name="email" class="form-control" autocomplete="off" value="{{ $clients->email }}">
                </div>
                <div class="form-group">
                    <label for="exampleFormControlInput1">Mot de Pass</label>
                    <input type="text" name="pass" class="form-control" value="{{ $clients->pass }}">
                </div>
